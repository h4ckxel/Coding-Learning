...
def preprocess_frame(frame):
    ...
    if result.multi_hand_landmarks:
        for hand_landmarks in result.multi_hand_landmarks:
            # Dibujar las marcas de la mano (opcional)
            mp_drawing.draw_landmarks(frame, hand_landmarks, mp_hands.HAND_CONNECTIONS)
            
            # Recortar la región de la mano (bounding box)
            h, w, c = frame.shape
...
while True:
    ...
    if hand_frame is not None:
        ...
        # Mapear la clase predicha a una etiqueta (por ejemplo, "hola", "gracias", etc.)
        class_labels = ["hola", "gracias", "por favor", "adios"]  # Ejemplo de etiquetas
